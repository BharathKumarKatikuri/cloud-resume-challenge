...
### Step 17: Create a Hosted Zone in Route 53

Created a public hosted zone for the custom domain cloudbuilders.site.
...
with Route 53 nameservers.

### Step 19: Request SSL Certificate using ACM

CloudFront requires certificates to be created in the us-east-1 (N. Virginia) region.
...
This created CNAME records similar to:

_xxxxx.cloudbuilders.site
→
_xxxxx.acm-validations.aws

After DNS propagation, certificate status changed from:
...
Issued
### Step 21: Configure CloudFront with Custom Domain

Updated the CloudFront distribution settings.
...
Saved changes and waited for CloudFront deployment.

Step 22: Create Route 53 Alias Record

Created an Alias A record pointing the domain to CloudFront.
...
Selected the CloudFront distribution.

Step 23: Verify HTTPS and Custom Domain

Verified that the website was accessible through:
...
✅ S3 website accessible through CloudFront
